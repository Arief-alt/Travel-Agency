...
export const getUsersAndTripsStats = async (): Promise<DashboardStats> => {
    const d = new Date()
    const startCurr = new Date(d.getFullYear(), d.getMonth(), 1).toISOString()
    ...
    const filterByDate: FilterByDate = (items, key, start, end) => items.filter((item) => (
        item[key] >= start && (end ? item[key] <= end : true)
    )).length
...
export const getTripsCreatedByUser = async () => {
    try {
        ...
        console.log('getTripsCreatedByUser error:', e);
    }
}